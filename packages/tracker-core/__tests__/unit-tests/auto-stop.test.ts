...
describe('autoStopNotice', () => {
  ...
  it('says nothing before the portal has answered', () => {
    expect(autoStopNotice(t, null, ZONE, 'idle', kolkata(11))).toBeNull();
  });

  it('states the stop time while the day is still young', () => {
    const notice = autoStopNotice(t, SETTINGS, ZONE, 'tracking', kolkata(11));
  ...
  it('warns as the window is about to shut on a running session', () => {
    const notice = autoStopNotice(t, SETTINGS, ZONE, 'tracking', kolkata(17, 52));
  ...
  it('warns a paused session too — it is just as stopped at six', () => {
    expect(autoStopNotice(t, SETTINGS, ZONE, 'paused', kolkata(17, 52))?.severity).toBe('warning');
  });

  it('does not count down at an idle tracker that has nothing to lose yet', () => {
    const notice = autoStopNotice(t, SETTINGS, ZONE, 'idle', kolkata(17, 52));

    expect(notice?.severity).toBe('info');
  });

  it('warns that a session started outside the window will be stopped again', () => {
    const notice = autoStopNotice(t, SETTINGS, ZONE, 'idle', kolkata(20));
  ...
  it('reads the clock in the EMPLOYEE’s zone, not this machine’s', () => {
    ...
    expect(autoStopNotice(t, SETTINGS, ZONE, 'idle', at)?.severity).toBe('warning');
    expect(autoStopNotice(t, SETTINGS, 'UTC', 'idle', at)?.severity).toBe('info');
  });
  ...
  it('counts a night shift’s stop hour as tomorrow’s, not a negative countdown', () => {
    ...
    const notice = autoStopNotice(t, nightShift, ZONE, 'tracking', kolkata(23));
