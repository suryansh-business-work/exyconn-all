...
/**
 * Rich text written in the portal (policies, consent notices) is rendered with
 * `dangerouslySetInnerHTML`, so it is sanitised in the browser first. The server cleans the
 * body when it is saved as well; this is the second layer, for rows written before that or
 * by any path that skipped it.
 *
 * The allow-list is what the rich-text editor produces: headings, paragraphs, lists (with
 * check lists), tables, quotes, code, links and images, plus alignment and colour as inline
 * styles. Links may only use http, https, mailto and tel; images only https.
 */
// prettier-ignore
const ALLOWED_TAGS = [
...
/** The HTML with everything outside the rich-text allow-list removed. */
