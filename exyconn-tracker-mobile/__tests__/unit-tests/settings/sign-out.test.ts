...
describe('signOutMessage', () => {
  it('only reminds about signing in again when nothing is open or pending', () => {
    expect(signOutMessage(t, 'idle', 0)).toBe(SIGN_IN_AGAIN);
  });

  it('warns that a running session stops', () => {
    expect(signOutMessage(t, 'tracking', 0)).toMatch(/^Tracking is running\. Signing out stops it/);
  });

  it('warns that a paused session ends', () => {
    expect(signOutMessage(t, 'paused', 0)).toMatch(/^Your paused session ends/);
  });

  it('counts the work still waiting to upload', () => {
    expect(signOutMessage(t, 'idle', 1)).toMatch(/^1 item is still waiting to upload/);
    expect(signOutMessage(t, 'idle', 3)).toMatch(/^3 items are still waiting to upload/);
  });

  it('puts every warning in its own paragraph, the reminder last', () => {
    const paragraphs = signOutMessage(t, 'tracking', 2).split('\n\n');
