...
import { brandColors, resolveScheme, toHex, type BrandColors } from './brand';
import { FALLBACK_BRAND } from './palette';
...
export function BrandProvider({ branding, themeMode, groundOpacity, children }: Readonly<Props>) {
  ...
  const value = useMemo<BrandContextValue>(() => {
    // The scheme first: the brand colour is made readable on THAT scheme's panel.
    const background = toHex(branding?.backgroundColor, FALLBACK_BRAND.background);
    const scheme = resolveScheme(themeMode, background, systemPrefersDark);
    return {
      ...brandColors(branding, scheme),
      scheme,
      branding,
      groundOpacity,
    };
