 ...
 *
 * The same contract as the portal's `useMissingStrings`, over IPC instead of Apollo: the
 * screen renders in English immediately and swaps when the catalogue lands, and every string
 * it could not translate is reported so the NEXT launch reads in the employee's own language
 * without anybody having prepared a catalogue.
 */
export default function useTrackerTranslations(locale: string): {
  messages: Messages;
