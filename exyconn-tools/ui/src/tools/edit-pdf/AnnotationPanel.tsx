...
const AnnotationPanel: React.FC<AnnotationPanelProps> = ({
  ...
  <>
    <Paper sx={{ p: 3 }}>
      ...
      {clickPos && (
        <Typography variant="body2" sx={{ mt: 1 }}>
          Position: ({clickPos.x}, {clickPos.y}) on page {currentPage}
        </Typography>
      )}
      {!clickPos && (
        <Typography
        ...
        >
          Click on the PDF preview to set position
        </Typography>
      )}
      <Button
        variant="outlined"
    ...
    <Button
      ...
      startIcon={<Download />}
      onClick={onDownload}
      disabled={!annotations.length || processing}
