...
const ACCEPTED = '.doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document';
...
export default function WordToPdf() {
  ...
  const loadFile = useCallback((f: File) => {
    ...
    if (!ext.endsWith('.doc') && !ext.endsWith('.docx')) { setError('Please select a Word document (.doc or .docx).'); return; }
    setInfo({ name: f.name, size: f.size, type: f.type || 'application/msword', lastModified: new Date(f.lastModified).toLocaleDateString() });
    setFile(f);
  }, []);
  ...
  const tips = [
    { primary: 'Google Docs', secondary: 'Upload to Google Docs → File → Download → PDF Document (.pdf)' },
    { primary: 'Microsoft Word', secondary: 'Open in Word → File → Save As → Choose PDF format' },
    { primary: 'Print to PDF', secondary: 'Open the document → Ctrl+P → Select "Microsoft Print to PDF"' },
    { primary: 'LibreOffice (Free)', secondary: 'Open in LibreOffice Writer → File → Export as PDF' },
  ];

  return (
    <ToolLayout toolName="Word to PDF" toolIcon={<Description />} toolColor="#2563eb">
      <Container maxWidth="xl" sx={{ py: 3 }}>
        <Grid container spacing={3}>
          <Grid size={{ xs: 12, md: 6 }}>
            <Paper
              sx={{ p: 4, textAlign: 'center', border: '2px dashed', borderColor: dragOver ? '#2563eb' : 'divider', cursor: 'pointer', transition: '0.2s' }}
            ...
            >
              ...
              <Button variant="outlined" component="label" sx={{ color: '#2563eb', borderColor: '#2563eb' }}>
            ...
            {info && file ? (
              <Paper sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                  ...
                  <Chip label="Coming Soon" color="primary" size="small" variant="outlined" />
                </Box>
                <Table size="small">
                  <TableBody>
                ...
                <Button variant="contained" fullWidth startIcon={<Download />} onClick={downloadFile}
                  sx={{ mt: 2, bgcolor: '#2563eb', '&:hover': { bgcolor: '#1d4ed8' } }}>Download Original File</Button>
              </Paper>
            ) : (
              <Paper sx={{ p: 3 }}>
