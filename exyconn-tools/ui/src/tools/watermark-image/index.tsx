...
export default function WatermarkImage() {
  ...
  const loadFile = useCallback(async (f: File) => {
    ...
    try {
      setBaseImage(await loadImageFromFile(f));
      setFile(f);
    } catch {
  ...
  const loadWatermarkFile = useCallback(async (f: File) => {
    ...
    try {
      setWatermarkImage(await loadImageFromFile(f));
  ...
  useEffect(() => {
    ...
    if (mode === 'image' && watermarkImage)
      drawImageWatermark(ctx, canvas.width, canvas.height, watermarkImage, imageOptions);
  ...
  const download = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = watermarkedFileName(file.name);
      a.click();
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            <Paper sx={{ p: 3 }}>
              ...
              <Tabs
                value={mode}
                onChange={(_, v: WatermarkMode) => setMode(v)}
                sx={{ mb: 2 }}
                slotProps={{
              ...
              <WatermarkControls
                ...
                onWatermarkFile={loadWatermarkFile}
              />
              <Button
                variant="contained"
                fullWidth
