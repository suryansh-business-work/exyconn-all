...
const SYSTEM_PROMPT = `You are an expert FAQ content generator. Generate comprehensive, accurate FAQs based on PDF document content.
...
const PdfFAQGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!content) throw new Error('No content found in the PDF');

      const userPrompt = `Generate ${count} FAQs in a ${tone} tone based on this PDF content:\n\n${content.substring(0, 8000)}`;
  ...
  return (
    <ToolLayout toolName="PDF FAQ Generator" toolIcon={<PictureAsPdf />} toolColor="#ef4444">
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            ...
            <Paper elevation={0} sx={{ border: 1, borderColor: 'divider', p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                ...
                <Typography variant="h6" fontWeight={600}>Upload PDF</Typography>
              </Box>
            ...
            {needsKey && (
              <Box sx={{ mt: 2 }}>
                <MissingKeyAlert
                  ...
                  hint="FAQs are written by OpenAI from the extracted PDF text using your own key."
                />
              </Box>
            )}
          </Grid>
