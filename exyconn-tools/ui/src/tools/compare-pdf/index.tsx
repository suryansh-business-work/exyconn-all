    ...
    >
      ...
      {file && (
        <Typography variant="body2" sx={{ mt: 1 }}>
          <strong>{file.name}</strong> ({fmt(file.size)})
        </Typography>
      )}
    </Paper>
  );
...
export default function ComparePdf() {
  ...
  const load1 = useCallback((f: File) => {
  ...
  const load2 = useCallback((f: File) => {
    if (f.type !== 'application/pdf') {
      setError('PDF only');
      return;
    }
  ...
  const compare = async () => {
    ...
    try {
      ...
      setError('Failed to load one or both PDFs.');
    } finally {
      setProcessing(false);
    }
  };

  const rows: { prop: string; v1: string; v2: string }[] =
    meta1 && meta2
      ? [
          ...
          ...(meta2.dims.length > meta1.dims.length
            ? meta2.dims
                .slice(meta1.dims.length)
                .map((d, i) => ({ prop: `Page ${meta1.dims.length + i + 1} Size`, v1: 'N/A', v2: d }))
            : []),
        ]
      : [];
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={12}>
            ...
            <Button
              ...
              sx={{ bgcolor: COLOR, '&:hover': { bgcolor: '#0d9488' }, mb: 3 }}
            >
