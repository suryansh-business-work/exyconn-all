...
export default function ExcelToPdf() {
  ...
  const loadFile = useCallback((f: File) => {
    if (!isAcceptedFile(f.name)) { setFileError('Please select an Excel file (.xls or .xlsx).'); return; }
  ...
  return (
    <ToolLayout toolName="Excel to PDF" toolIcon={<FaFileExcel />} toolColor={COLOR}>
        ...
        <Grid container spacing={3}>
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            >
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>Supports .xls and .xlsx files</Typography>
            ...
            {file ? (
              ...
              <Paper sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 4 }}>
                  <Box sx={{ fontSize: 64, color: 'text.disabled', mb: 2, display: 'flex' }}><FaFileExcel /></Box>
                  <Typography color="text.secondary">Upload an Excel spreadsheet to get started</Typography>
                </Box>
              </Paper>
            )}
          </Grid>
