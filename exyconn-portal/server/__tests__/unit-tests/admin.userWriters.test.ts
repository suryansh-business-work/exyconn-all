...
describe('who may assign which roles', () => {
  it('lets HR create an ordinary employee', () => {
    expect(() => assertMayAssignRoles(hr, [ROLES.EMPLOYEE, ROLES.SUPPORT])).not.toThrow();
  });

  it('stops HR granting the ADMIN role', () => {
    expect(() => assertMayAssignRoles(hr, [ROLES.EMPLOYEE, ROLES.ADMIN])).toThrow(
  ...
  it('stops HR editing an existing administrator at all', () => {
    ...
    expect(() => assertMayAssignRoles(hr, undefined, { roles: [ROLES.ADMIN] })).toThrow(
  ...
  it('lets an ADMIN do both', () => {
    expect(() =>
      assertMayAssignRoles(admin, [ROLES.ADMIN], { roles: [ROLES.ADMIN] }),
    ).not.toThrow();
  });

  it('allows an edit that does not touch roles', () => {
    expect(() => assertMayAssignRoles(hr, undefined, { roles: [ROLES.EMPLOYEE] })).not.toThrow();
  });
});
