...
/** How far ahead the probation panel looks when the caller does not say. */
...
export const hrCustomResolvers = {
  Query: {
    ...
    probationsEnding: async (
      _p: unknown,
      { days }: { days?: number | null },
      ctx: GraphQLContext,
    ) => {
      assertRole(ctx, hrOnly);
      return withIds(await hrService.probationsEnding(days ?? DEFAULT_PROBATION_WINDOW_DAYS));
    },
    teamLeaveRequests: async (_p: unknown, _a: unknown, ctx: GraphQLContext) => {
      const user = assertAuthenticated(ctx);
      return withIds(await hrService.teamLeaves(await directReportIds(user.id)));
    },
  },
  Mutation: {
