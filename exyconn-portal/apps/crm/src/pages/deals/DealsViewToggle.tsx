  ...
  return (
    ...
    >
      <ToggleButtonGroup
        ...
        exclusive
        value={view}
        aria-label={t('deals view')}
        onChange={(_event, next: 'board' | 'list' | null) => {
          if (next === 'board') navigate(DEALS_BOARD_PATH);
          if (next === 'list') navigate(DEALS_LIST_PATH);
        }}
      >
        <ToggleButton value="board" aria-label={t('board view')}>
          <ViewKanbanIcon fontSize="small" sx={{ mr: 0.5 }} /> {t('Board')}
        </ToggleButton>
        <ToggleButton value="list" aria-label={t('list view')}>
          <TableRowsIcon fontSize="small" sx={{ mr: 0.5 }} /> {t('List')}
        </ToggleButton>
      </ToggleButtonGroup>
    </Stack>
  );
