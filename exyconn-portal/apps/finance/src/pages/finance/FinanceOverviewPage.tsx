...
import { useListInvoicesQuery, useListInvoicesStatsQuery } from '@exyconn/shell/graphql/generated';
...
/** An invoice is overdue once its due date has passed and it is still unpaid. */
function isOverdue(invoice: { status: string; dueDate?: string | null }): boolean {
  return (
    invoice.status !== 'PAID' && Boolean(invoice.dueDate) && new Date(invoice.dueDate!) < new Date()
  );
}
...
export function FinanceOverviewPage() {
  ...
  const statItems: StatItem[] = [
    ...
    { label: 'Overdue', value: String(overdue.length), accent: '#ff6b6b' },
    { label: 'Invoiced', value: formatMoney(statSum(stats, 'amount')), accent: '#8b5cf6' },
  ];
