...
export function AiJobResult({ id }: Readonly<{ id: string }>) {
  ...
  if (!job) {
    return <Text size="sm">{loading ? t('Loading…') : t('This job is no longer available.')}</Text>;
  }

  const notRunYet = t('Not run yet');
  const tokenLine = t('{total} ({prompt} prompt + {completion} completion)', {
    total: job.totalTokens.toLocaleString(),
    prompt: job.promptTokens.toLocaleString(),
    completion: job.completionTokens.toLocaleString(),
  });
  ...
  return (
    <Flex direction="column" spacing={1.5}>
      ...
      <DetailRow label={t('Run by')}>
        <Text size="sm">{job.createdByName || notRunYet}</Text>
      </DetailRow>
      <DetailRow label={t('Tokens')}>
        <Text size="sm">{tokenLine}</Text>
      </DetailRow>
      <DetailRow label={t('Cost')}>
        <Text size="sm">{formatCostUsd(job.costUsd, Boolean(job.ranAt))}</Text>
      </DetailRow>
      <DetailRow label={t('Took')}>
        <Text size="sm">{(job.latencyMs / MS_PER_SECOND).toFixed(2)}s</Text>
      </DetailRow>
      <DetailRow label={t('Last run')}>
        <Text size="sm">{job.ranAt ? formatDateTime(job.ranAt) : notRunYet}</Text>
      </DetailRow>

      <Divider />
      <Text size="label">{t('Prompt')}</Text>
      <Text size="sm" sx={{ whiteSpace: 'pre-wrap' }}>
        {job.prompt}
      </Text>

      <Divider />
      <Text size="label">{t('Response')}</Text>
      <AiJobProgress status={job.status} queuedAt={job.queuedAt} />
      {job.error && <Alert severity="error">{job.error}</Alert>}
      {!waiting && !job.error && (
        <Text size="sm" sx={{ whiteSpace: 'pre-wrap' }}>
          {job.response || t('Nothing yet — run the job to get an answer.')}
        </Text>
      )}
    </Flex>
  );
